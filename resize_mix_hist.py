...
def hist(img):
    a, b = numpy.histogram(img, bins=numpy.arange(1.1,step=0.1))
    total_count = float(numpy.sum(a))
    hist = [int(x / total_count * 100) for x in a]
    return hist
...
def get_features(filename):
    ...
    for channel in range(0, 4):
        ...
        histogram = histogram + hist(image)
            ...
            for is_incrase_contrast in range(1, 2):
                ...
                for is_otsu in range(0, 2):
                    ...
                    for sigma in param_sigma:
                        feature.append(get_num_edges(image_gray_3, sigma))
                """

    out = ""
    feature = histogram + feature
